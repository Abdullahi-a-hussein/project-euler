def largest_product(grid, n):
    return max(vertical_horizontal_product(grid, n), diagnols_product(grid, n))


def vertical_horizontal_product(grid, n):
    max_product = 0
...
def diagnols_product(grid, n):

    max_product = 0
    # top to buttom
    i = 0
    while i <= len(grid[0]) - n:
        row = i
        col = 0
        while row < len(grid) - n and col < len(grid[0]) - n:
            product1 = 1
            product2 = 1
            for j in range(n):
                product1 *= grid[row + j][col + j]
                product2 * grid[col + j][row + j]
    ...
    while i > n:
        ...
        k = 0
        while k < len(grid) - n:
            while col <= (len(grid) - n) and row >= 4:
                product = 1
                for j in range(n):
